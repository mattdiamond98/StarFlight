...
public class MediumEnemyAI extends ArtificialIntelligence {
	...
	protected void onUpdate(GameContainer gc, Ship s, int i) {
		...
		s.a = Math.toDegrees(Math.atan2(p.y - s.y - (p.size / 2), p.x - s.x
				- (p.size / 2)));
		if (aiRangeOf(0, 50)) {
		...
		if (aiRangeOf(150, 200))
			s.shoot(gc, 0);
		if (aiRangeOf(200, 250))
			if (s.v.lengthSquared() < s.MAX_V * s.MAX_V) {
				s.v.x += 1.25 * Math.sin(-1 * Math.toRadians(s.a));
				s.v.y += 1.25 * Math.cos(-1 * Math.toRadians(s.a));
				s.shoot(gc, 1);
			}
		if (aiRangeOf(250, 300)) {
			if (s.v.lengthSquared() < s.MAX_V * s.MAX_V) {
